...
We would like a list of count-paired domains, (in the same format as
the input, and in any order), that explicitly counts the number of visits
to each subdomain.
...
class Solution:
    def subdomainVisits(self, cpdomains: List[str]) -> List[str]:
        ...
        for cpdomain in cpdomains:
            ...
            visit, n = int(visit), len(domain)
            for i in range(-1, n):
                if i == -1 or domain[i] == ".":
                    counter[domain[i+1:]] += visit
        return [f"{counter[domain]} {domain}" for domain in counter]
