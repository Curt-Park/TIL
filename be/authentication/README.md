...
| [02](02-session/) | Session | 비밀번호가 요청마다 오간다. 거쳐 가는 지점마다 노출되는데, 한 번 새면 계정 자체가 넘어간다 | 서버가 뽑아서 기억하는 무작위 ID로 비밀번호를 대신하고, 필요하면 그것을 잊어버린다 | `Authorization: Bearer`로의 전환, 로그아웃과 만료, session fixation 방어, in-memory 저장소의 한계 |
