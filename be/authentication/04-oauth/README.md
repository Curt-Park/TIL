...
`code`를 가로챈 쪽은 그 값을 알 수 없으므로 교환에 실패한다. 이것이 PKCE다.
...
브라우저에 값을 저장할 방법을 정한 적이 없다. 여기서 두 가지 문제가 발생한다.
...
지금 `state`는 한 경로로만 다닌다. 앱이 URL에 실어 보내고, 그 URL이 제공자를 거쳐 그대로 돌아온다.
공격자가 그 URL을 만들어 남에게 열게 하면 앱은 구별하지 못한다.

같은 값이 서로 다른 두 경로로 도착해야 걸러진다. 하나는 지금처럼 URL이고, 다른 하나는 로그인을 시작할 때
브라우저에 저장해 두었다가 돌려받는 경로다. 앱은 `/callback`에서 이 둘을 대조한다.

```
GET /callback?state=ABC123        <- 공격자가 만들어 건넨 URL의 값
브라우저가 돌려보낸 값: 없음        <- 공격자는 남의 브라우저에 값을 저장시킬 수 없다
```

공격자가 자기 브라우저에 무엇을 저장해 두든 소용이 없다. 그 값은 공격자의 브라우저에 남아 있고,
주소를 여는 것은 피해자의 브라우저이기 때문이다.
브라우저에 값을 저장시켰다가 돌려받는 수단이 없어서 지금은 이 대조를 수행할 수 없다.
...
두 문제 모두 브라우저에 값을 저장시키고 서버가 그것을 돌려받는 수단이 있으면 해결된다.
