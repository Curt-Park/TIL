...
in s are strictly increasing from left to right
...
class Solution:
    def areNumbersAscending1(self, s: str) -> bool:
        """Solution For Python3.10: O(N) / O(N)."""
        return all(
            d0 < d1 for d0, d1 in itertools.pairwise(
                [int(w) for w in s.split() if w.isdigit()]
            )
        )

    def areNumbersAscending0(self, s: str) -> bool:
        """O(N) / O(N)"""
        prev_token = -1
        for w in s.split():
            if not w.isdigit():
